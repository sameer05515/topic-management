...
app
		.controller(
				...
				function($scope/* , $http */, $log, $location, $routeParams,
						topicMgmtAppConfig, TasksManagementServices) {
					...
					$scope.maxRatingValue=TasksManagementServices.maxTopicMgmtRatingValue;
					...
					$scope.fetchGroupList = function() {
						$scope.groupsList = [];
						TasksManagementServices.fetchGroupList().success(
								function(data) {
					 ...
					 * topicGroupRelationResource.id.push($routeParams.id);
					 * 
					 * TasksManagementServices .fetchTopicGroupRelationList(
